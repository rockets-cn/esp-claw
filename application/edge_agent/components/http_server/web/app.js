...
const strings = {
  en: {
    ...
    enabledCapGroups: "Enabled Capability Groups",
    enabledCapGroupsHelp:
      "Choose which compiled capability groups can be used after boot, and which enabled groups stay visible to the LLM. Changes take effect after restart.",
    capabilitySelectAll: "Select All",
    capabilityClearAll: "Clear All",
    capabilityLlmDefaults: "Restore LLM Set",
    capabilityLlmClear: "Clear LLM",
    capabilityLoading: "Loading capabilities...",
    capabilityLoadError: "Failed to load capabilities",
    capabilitySelectedSummary: "{enabled} enabled, {llm} LLM-visible, total {total}",
    capabilityEnabled: "Enabled",
    capabilityLlmAlwaysVisible: "LLM Always Visible",
    enabledLuaModules: "Enabled Lua Modules",
    enabledLuaModulesHelp:
      "Choose which compiled Lua modules are registered when cap_lua starts. Changes take effect after restart.",
    luaModuleSelectAll: "Select All",
    luaModuleClearAll: "Clear All",
    luaModuleLoading: "Loading Lua modules...",
    luaModuleLoadError: "Failed to load Lua modules",
    luaModuleSelectedSummary: "{enabled} enabled, total {total}",
    luaModuleCapabilityRequired:
      "Enable the cap_lua capability group above before Lua module settings can take effect.",
    timezone: "Timezone",
    ...
    configNote:
      "Changes are stored in NVS. Choose an LLM provider first, then fill the shared API key and model fields. Restart the device after updating Wi-Fi, Feishu, core LLM, capability, or Lua module settings.",
  ...
  "zh-cn": {
    ...
    enabledCapGroups: "启用的 Capability Group",
    enabledCapGroupsHelp:
      "选择设备启动后允许使用哪些已编译 capability group，以及哪些启用后的 group 对 LLM 始终可见。修改后需重启生效。",
    ...
    capabilitySelectedSummary: "已启用 {enabled}，LLM 可见 {llm}，总计 {total}",
    capabilityEnabled: "启用",
    capabilityLlmAlwaysVisible: "LLM 始终可见",
    enabledLuaModules: "启用的 Lua 模块",
    enabledLuaModulesHelp:
      "选择 cap_lua 启动时要注册哪些已编译 Lua 模块。修改后需重启生效。",
    luaModuleSelectAll: "全选",
    luaModuleClearAll: "清空",
    luaModuleLoading: "正在加载 Lua 模块...",
    luaModuleLoadError: "加载 Lua 模块列表失败",
    luaModuleSelectedSummary: "已启用 {enabled}，总计 {total}",
    luaModuleCapabilityRequired:
      "需先在上方启用 cap_lua capability group，Lua 模块配置才会生效。",
    timezone: "时区",
    ...
    configNote:
      "更改存储在 NVS 中。请先选择 LLM 提供商，然后填写 API 密钥和模型字段。更新 Wi-Fi、飞书、核心 LLM、capability 或 Lua 模块设置后请重启设备。",
...
const CAPABILITY_NONE_SENTINEL = "__none__";
let capabilityItems = [];
let pendingEnabledCapGroups = "";
let pendingLlmVisibleCapGroups = "";
let luaModuleItems = [];
let pendingEnabledLuaModules = "";
...
function readConfigForm() {
  ...
  payload.enabled_cap_groups = serializeCapabilitySelection();
  payload.llm_visible_cap_groups = serializeLlmVisibleSelection();
  payload.enabled_lua_modules = serializeLuaModuleSelection();
  return payload;
}
...
function fillConfigForm(data) {
  ...
  pendingEnabledCapGroups =
    typeof data.enabled_cap_groups === "string" ? data.enabled_cap_groups : "";
  pendingLlmVisibleCapGroups =
    typeof data.llm_visible_cap_groups === "string"
      ? data.llm_visible_cap_groups
      : "";
  pendingEnabledLuaModules =
    typeof data.enabled_lua_modules === "string" ? data.enabled_lua_modules : "";
  renderCapabilityCheckboxes();
  renderLuaModuleCheckboxes();
  syncProviderPreset();
}
...
function getSelectionSet(
  ...
  if (!raw) {
    items.forEach((item) => {
      if (selectAllIfEmpty || (defaultPredicate && defaultPredicate(item))) {
        selected.add(item[idKey]);
      }
    });
    return selected;
  }
  ...
  raw.split(",").forEach((token) => {
    const itemId = token.trim();
    if (!itemId || itemId === CAPABILITY_NONE_SENTINEL || itemId === "none") {
      return;
    }
    selected.add(itemId);
  });
...
function getCapabilitySelectionSet(serialized, selectAllIfEmpty, useDefaultLlmIfEmpty) {
  return getSelectionSet(
    serialized,
    capabilityItems,
    "group_id",
    selectAllIfEmpty,
    useDefaultLlmIfEmpty ? (item) => item.default_llm_visible : null
  );
}

function getLuaModuleSelectionSet(serialized, selectAllIfEmpty) {
  return getSelectionSet(serialized, luaModuleItems, "module_id", selectAllIfEmpty);
}

function updateCapabilitySelectionSummary() {
  const summary = document.getElementById("capabilitySelectionSummary");
  const enabledCount = document.querySelectorAll(
    '#capabilityCheckboxList input[data-role="enabled"]:checked'
  ).length;
  const llmCount = document.querySelectorAll(
    '#capabilityCheckboxList input[data-role="llm-visible"]:checked'
  ).length;

  if (!summary) return;
  if (!capabilityItems.length) {
    summary.textContent = t("capabilityLoading");
    return;
  }

  summary.textContent = formatTemplate("capabilitySelectedSummary", {
...
function isLuaCapabilityEnabled() {
  return !!document.querySelector(
    '#capabilityCheckboxList input[data-role="enabled"][value="cap_lua"]:checked'
  );
}

function updateLuaModuleAvailability() {
  const disabled = !isLuaCapabilityEnabled();
  const list = document.getElementById("luaModuleCheckboxList");
  const note = document.getElementById("luaModuleCapabilityNote");
  const selectAllButton = document.getElementById("luaModuleSelectAllButton");
  const clearAllButton = document.getElementById("luaModuleClearAllButton");

  if (list) {
    list.classList.toggle("is-disabled", disabled);
  }
  if (note) {
    note.classList.toggle("hidden", !disabled);
  }
  [selectAllButton, clearAllButton].forEach((button) => {
    if (button) {
      button.disabled = disabled || !luaModuleItems.length;
    }
  });
  document
    .querySelectorAll('#luaModuleCheckboxList input[data-role="enabled"]')
    .forEach((input) => {
      input.disabled = disabled;
    });
}

function updateLuaModuleSelectionSummary() {
  const summary = document.getElementById("luaModuleSelectionSummary");
  const enabledCount = document.querySelectorAll(
    '#luaModuleCheckboxList input[data-role="enabled"]:checked'
  ).length;

  if (!summary) return;
  if (!luaModuleItems.length) {
    summary.textContent = t("luaModuleLoading");
    return;
  }

  summary.textContent = formatTemplate("luaModuleSelectedSummary", {
    enabled: enabledCount,
    total: luaModuleItems.length,
  });
}

function renderCapabilityCheckboxes() {
  const list = document.getElementById("capabilityCheckboxList");
  if (!list) return;

  list.innerHTML = "";
  if (!capabilityItems.length) {
    updateCapabilitySelectionSummary();
    return;
  }

  const enabled = getCapabilitySelectionSet(pendingEnabledCapGroups, true, false);
  const llmVisible = getCapabilitySelectionSet(
    pendingLlmVisibleCapGroups,
    false,
    true
  );
  capabilityItems.forEach((item) => {
    ...
    title.textContent = item.display_name || item.group_id;

    const id = document.createElement("div");
    id.className = "capability-id";
    id.textContent = item.group_id;
    ...
    enabledToggle.className = "capability-toggle";
    const enabledInput = document.createElement("input");
    enabledInput.type = "checkbox";
    enabledInput.value = item.group_id;
    enabledInput.dataset.role = "enabled";
    enabledInput.checked = enabled.has(item.group_id);
    enabledToggle.appendChild(enabledInput);
    enabledToggle.appendChild(document.createTextNode(t("capabilityEnabled")));

    const llmToggle = document.createElement("label");
    llmToggle.className = "capability-toggle";
    const llmInput = document.createElement("input");
    llmInput.type = "checkbox";
    llmInput.value = item.group_id;
    llmInput.dataset.role = "llm-visible";
    llmInput.checked = enabled.has(item.group_id) && llmVisible.has(item.group_id);
    llmInput.disabled = !enabledInput.checked;
    llmToggle.appendChild(llmInput);
    llmToggle.appendChild(
      document.createTextNode(t("capabilityLlmAlwaysVisible"))
    );

    enabledInput.addEventListener("change", () => {
      if (!enabledInput.checked) {
        llmInput.checked = false;
        llmInput.disabled = true;
      } else {
        llmInput.disabled = false;
      }
      updateCapabilitySelectionSummary();
      updateLuaModuleAvailability();
    });
    llmInput.addEventListener("change", updateCapabilitySelectionSummary);
...
function serializeCapabilitySelection() {
  const checked = Array.from(
    document.querySelectorAll('#capabilityCheckboxList input[type="checkbox"]:checked')
  ).map((input) => input.value);

  if (capabilityItems.length && checked.length === capabilityItems.length) {
    return "";
  }
  if (checked.length === 0) {
    return CAPABILITY_NONE_SENTINEL;
  }
  return checked.join(",");
}

function setAllCapabilityCheckboxes(checked) {
  document
    .querySelectorAll('#capabilityCheckboxList input[data-role="enabled"]')
    .forEach((input) => {
      input.checked = checked;
    });
  document
    .querySelectorAll('#capabilityCheckboxList input[data-role="llm-visible"]')
    .forEach((input) => {
...
function serializeLlmVisibleSelection() {
  const checked = Array.from(
    document.querySelectorAll(
      '#capabilityCheckboxList input[data-role="llm-visible"]:checked'
    )
  ).map((input) => input.value);
  const defaultSelected = capabilityItems
    .filter((item) => item.default_llm_visible)
    .map((item) => item.group_id);

  if (
    checked.length === defaultSelected.length &&
    checked.every((groupId) => defaultSelected.includes(groupId))
  ) {
    return "";
  }
  if (checked.length === 0) {
...
function setLlmVisibleSelection(mode) {
  const enabled = new Set(
    Array.from(
      document.querySelectorAll('#capabilityCheckboxList input[data-role="enabled"]:checked')
    ).map((input) => input.value)
  );
  ...
  document
    ...
    .forEach((input) => {
      ...
      if (mode === "defaults") {
        const item = capabilityItems.find((entry) => entry.group_id === input.value);
        input.checked = !!item?.default_llm_visible;
      } else if (mode === "clear") {
        input.checked = false;
      }
    });
...
function renderLuaModuleCheckboxes() {
  const list = document.getElementById("luaModuleCheckboxList");
  if (!list) return;

  list.innerHTML = "";
  if (!luaModuleItems.length) {
    updateLuaModuleSelectionSummary();
    updateLuaModuleAvailability();
    return;
  }

  const enabled = getLuaModuleSelectionSet(pendingEnabledLuaModules, true);
  luaModuleItems.forEach((item) => {
    const label = document.createElement("label");
    label.className = "capability-item";
    ...
    const title = document.createElement("div");
    title.className = "capability-title";
    title.textContent = item.display_name || item.module_id;

    const id = document.createElement("div");
    id.className = "capability-id";
    id.textContent = item.module_id;
    ...
    const enabledInput = document.createElement("input");
    enabledInput.type = "checkbox";
    enabledInput.value = item.module_id;
    enabledInput.dataset.role = "enabled";
    enabledInput.checked = enabled.has(item.module_id);
    enabledInput.addEventListener("change", updateLuaModuleSelectionSummary);
    enabledToggle.appendChild(enabledInput);
    enabledToggle.appendChild(document.createTextNode(t("capabilityEnabled")));
...
function serializeLuaModuleSelection() {
  if (!luaModuleItems.length) {
    return pendingEnabledLuaModules || "";
  }

  const checked = Array.from(
    document.querySelectorAll('#luaModuleCheckboxList input[data-role="enabled"]:checked')
  ).map((input) => input.value);

  if (luaModuleItems.length && checked.length === luaModuleItems.length) {
    return "";
  }
  if (checked.length === 0) {
    return CAPABILITY_NONE_SENTINEL;
...
function setAllLuaModuleCheckboxes(checked) {
  const disabled = !isLuaCapabilityEnabled();
  document
    .querySelectorAll('#luaModuleCheckboxList input[data-role="enabled"]')
    .forEach((input) => {
...
async function loadCapabilities() {
  ...
  if (!response.ok) throw new Error(t("capabilityLoadError"));
  const data = await response.json();
  capabilityItems = Array.isArray(data.items) ? data.items : [];
  renderCapabilityCheckboxes();
}

async function loadLuaModules() {
  const response = await fetch("/api/lua-modules", { cache: "no-store" });
  if (!response.ok) throw new Error(t("luaModuleLoadError"));
  const data = await response.json();
  luaModuleItems = Array.isArray(data.items) ? data.items : [];
  renderLuaModuleCheckboxes();
}
...
function bindEvents() {
  initTabs();
  document
    .getElementById("capabilitySelectAllButton")
    .addEventListener("click", () => setAllCapabilityCheckboxes(true));
  document
    .getElementById("capabilityClearAllButton")
    .addEventListener("click", () => setAllCapabilityCheckboxes(false));
  document
    .getElementById("capabilityLlmSelectDefaultsButton")
    .addEventListener("click", () => setLlmVisibleSelection("defaults"));
  document
    .getElementById("capabilityLlmClearAllButton")
    .addEventListener("click", () => setLlmVisibleSelection("clear"));
  document
    .getElementById("luaModuleSelectAllButton")
    .addEventListener("click", () => setAllLuaModuleCheckboxes(true));
  document
    .getElementById("luaModuleClearAllButton")
    .addEventListener("click", () => setAllLuaModuleCheckboxes(false));
...
async function bootstrap() {
  ...
  try {
    ...
    showBanner("configBanner", err.message || t("capabilityLoadError"), true);
  }

  try {
    await loadLuaModules();
  } catch (err) {
    showBanner("configBanner", err.message || t("luaModuleLoadError"), true);
  }
