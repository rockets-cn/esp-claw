...
description: How `lua_module_*` components register, how to author them, and a full gpio module walkthrough.
---
...
`lua_module_*` components are ESP-Claw's **bridge from hardware peripherals or reusable Lua libraries to Lua scripts**.
C-backed modules register standard Lua C modules (`lua_CFunction` style) through `cap_lua_register_module`, then scripts load them with `require("module_name")`.
...
| `touch` | `lua_module_touch` | Capacitive touch key reads |
