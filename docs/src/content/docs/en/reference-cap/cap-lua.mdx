...
The `cap_lua` Skill instructs the model to check and stop running Lua async jobs before deactivating the Skill. At runtime, `cap_lua_async_jobs_provider` also injects the current async-job summary into context so the model can avoid leaving long-lived jobs behind.
