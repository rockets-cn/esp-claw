...
description: lua_module_* 组件的注册机制、实现方式，以及 gpio 模块的完整分析。
---
...
`lua_module_*` 是 ESP-Claw 中**将硬件外设能力或可复用 Lua 库暴露给 Lua 脚本**的扩展机制。
C-backed 模块通过 `cap_lua_register_module` 注册标准 Lua C 模块（`lua_CFunction` 格式），脚本中可用 `require("module_name")` 加载。
...
| `touch` | `lua_module_touch` | 电容触摸按键数据读取 |
